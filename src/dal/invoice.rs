...
pub async fn create_with_items(
    ...
    for (pos, (desc, qty, unit_amount)) in items.into_iter().enumerate() {
        ...
        sqlx::query("INSERT INTO line_items (id, invoice_id, description, quantity, unit_amount_cents, amount_cents, position) VALUES ($1, $2, $3, $4, $5, $6, $7)").bind(Ulid::new().to_string()).bind(&invoice.id).bind(desc).bind(qty).bind(unit_amount).bind(amount).bind(pos as i32).execute(&mut *tx).await?;
    }
    let created = sqlx::query_as::<_, Invoice>("INSERT INTO invoices (id, business_id, customer_id, total_cents, due_date, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *").bind(&invoice.id).bind(&invoice.business_id).bind(&invoice.customer_id).bind(invoice.total_cents).bind(invoice.due_date).bind(invoice.idempotency_key).fetch_one(&mut *tx).await?;
    tx.commit().await?;
    Ok(created)
}
...
pub async fn update_overdue(pool: &PgPool) -> Result<Vec<Invoice>, AppError> {
    let rows = sqlx::query_as::<_, Invoice>("UPDATE invoices SET state = $1, updated_at = NOW() WHERE state = $2 AND due_date < NOW() RETURNING *").bind("uncollectible").bind("open").fetch_all(pool).await?;
    Ok(rows)
}
